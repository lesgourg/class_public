...
def generate_dummy_live_data():
    import numpy as np
    network_names = ["t0_reco", "t0_reio", "t1", "t2_reco", "t2_reio"]
    result = []
    for name in network_names:
        d = {
            "network": name,
            "loss": 10**(-np.random.uniform(4, 9)),
            "forward": 1e-3 * np.random.uniform(0.5, 3.0),
            "backward": 1e-3 * np.random.uniform(0.5, 3.0),
        }
        result.append(d)
    return result


def generate_dummy_history(size=5):
    import numpy as np
    network_names = ["t0_reco_no_isw", "t0_reio_no_isw", "t0_isw", "t1", "t2_reco", "t2_reio", "phi_plus_psi"]
    return {n: np.random.uniform(0, 1.0, size=size) for n in network_names}
...
if __name__ == "__main__":
    ...
    with Dashboard() as dashboard:
        for epoch in range(n_epochs):
            for batch in range(0, n_batches, 5):
                dashboard.update(
                    "Training",
                    epoch, n_epochs,
                    batch, n_batches,
                    loading_time=0.00323,
                    live=generate_dummy_live_data(),
