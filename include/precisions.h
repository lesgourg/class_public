...
class_precision_parameter(accurate_lensing,int,_FALSE_) /**< switch between simple quadrature on a subdomain of angles (default) and accurate Gauss-Legendre quadrature integration. Accurate method is needed for precise predictions at l_max > 2500 and/or for B modes. It should be used when fitting accurate high-l CMB data (ACT, SPT, SO...) */
class_precision_parameter(num_mu_minus_lmax,int,0) /**< used only with accurate_lensing = _TRUE_. Difference between num_mu and l_max. Increase for more precision, but with a sufficient delta_l_max this is not really needed. New in v3.4: default is 0 instead of 70 (but this is compensated by a larger default value of delta_l_max) */
class_precision_parameter(delta_l_max,int,-1)/**< used only with both accurate_lensing = _FALSE) or _TRUE_. Defines by how much we should increase l_max_scalar in order to get sufficient precision on lensing up to the requested l_max. New in v3.4: instead of the defaut value of 500, we set this parameter to a preliminary -1, which means 'to be set automatically within the input module'. The user can still enter a numerical value for delta_l_max. If they don't, the input module will replace delta_l_max by l_max_scalars/n, where n=3 for accurate_lensing=_FALSE_ and n=2 for accurate_lensing=_TRUE_. With accurate_lensing=_TRUE_, this rule of thumb was tested to give ~0.1% precision up to l_max=8000. Note that when accurate BB modes are required, delta_l_max might need to be increased, certainly to at least 2000. (Credits: C. Pitrou) */
class_precision_parameter(tol_gauss_legendre,double,ppr->smallest_allowed_variation) /**< used only with accurate_lensing = _TRUE_. Tolerance with which quadrature points are found: must be very small for an accurate integration (if not entered manually, set automatically to match machine precision) */
class_precision_parameter(non_accurate_lensing_boundary,int,16) /**< used only with accurate_lensing = _FALSE_. Instead of integrating beta (the separation angle) from 0 to Pi, we integrate on [0, Pi/non_accurate_lensing_boundary]. Recommended value is 16 (see section III.B.3 of astro-ph/0502425). New in v3.4: the value 16 is no longer hard-coded. It can be changed by the user (with still 16 as the default). The truncation introduces artificial ringing on small scales. To avoid such ringing, especially with a large l_max, the user can reduce this value to 8, 4, 2, or even 1. Note however that for high precision the accurate_lensing method which uses a Gauss-Legendre quadrature is a better. It optimizes the number of points needed and avoids ringing. (Credits: C. Pitrou) */
class_precision_parameter(lensing_C0_order,int,0) /**< New in v3.4: lensing order of the monopolar contribution to the deflection correlation (called Cgl_0 in 0502425). Default is 0, that is the effect of Cgl_0 is ignored. Possible values: 0 (less precise), 1 (most precise) (Credits: C. Pitrou) */
class_precision_parameter(lensing_C2_order,int,2) /**< New in v3.4: lensing order of the quadrupolar contribution to the deflection correlation (called Cgl_2 in 0502425). Default is 2, that is up to Cgl_2^2. Possible values: 0 (less precise), 1, 2, 3, 4 (most precise) (Credits: C. Pitrou) */
