...
/**
 * This replaces perturb_k_list for debugging purpose.
 * The usual k-array is replaced by the fixed k-array of the ClassNet, 
 * loaded from a hardcoded workspace.
 * It should never be called for regular computation, as it is probably impacting the results!
 */

int perturb_get_k_list_net(
                       struct precision * ppr,
                       struct background * pba,
                       struct thermo * pth,
                       struct perturbs * ppt
  ...
  printf("WARNING: The k-array of ClassNet is used. This should only be done for debugging\n");

  file = fopen("/home/stadtmann/Class/CLASSnet_Workspace_new_2/data/k_binary","r"); 
  
  fseek(file,0,SEEK_END);
  file_size=ftell(file)/sizeof(double);
  ...
  newk_size=1;
  for (index_file=0; index_file<file_size; ++index_file){ 
  ...
  class_alloc(ppt->k[ppt->index_md_scalars],
	      newk_size*sizeof(double),
	      ppt->error_message);

  rewind(file);
  ppt->k[ppt->index_md_scalars][0]=k_min;
  index_k=1;
  for (index_file=0; index_file<file_size; ++index_file){ 
	  fread(&k,sizeof(double),1,file);
	  if (k>k_min){
  ...
  // get k_max_cl from the default get_k_list function
  if (_TRUE_){
    if (pba->sgnK == 0) {
      /* K<0 (flat)  : start close to zero */
      k_min=ppr->k_min_tau0/pba->conformal_age;
  ...
  index_k=0;
  while (ppt->k[ppt->index_md_scalars][index_k] < k_max_cl[ppt->index_md_scalars] && index_k < newk_size){
	  ++index_k;
          }
  ppt->k_size_cl[ppt->index_md_scalars] = index_k;
  ppt->k_size[ppt->index_md_scalars] = newk_size;
  ppt->k_min = ppt->k[ppt->index_md_scalars][0];
  ppt->k_max = ppt->k[ppt->index_md_scalars][ppt->k_size[ppt->index_md_scalars]-1];

  printf("MODF k_min: %e\n",ppt->k_min);
  printf("MODF k_max: %f\n",ppt->k_max);
  printf("MODF k_size: %d\n",ppt->k_size[ppt->index_md_scalars]);
  printf("MODF k_max_cl_calculated: %f\n",k_max_cl[ppt->index_md_scalars]);
  printf("MODF k_max_cl-1: %f\n",ppt->k[ppt->index_md_scalars][ppt->k_size_cl[ppt->index_md_scalars]]);
  printf("MODF k_size_cl: %d\n",ppt->k_size_cl[ppt->index_md_scalars]);
...
int perturb_get_k_list(
  ...
  */
  ...
  //printf("FULL k_min: %e\n",ppt->k_min);
  //printf("FULL k_max: %f\n",ppt->k_max);
  //printf("FULL k_size: %d\n",ppt->k_size[ppt->index_md_scalars]);
  //printf("FULL k_max_cl_calculated: %f\n",k_max_cl[ppt->index_md_scalars]);
  //printf("FULL k_max_cl-1: %f\n",ppt->k[ppt->index_md_scalars][ppt->k_size_cl[ppt->index_md_scalars]-1]);
  //printf("FULL k_size_cl: %d\n",ppt->k_size_cl[ppt->index_md_scalars]);
    ...
    */
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      delta_tot = (fracg*ppw->pv->y[ppw->pv->index_pt_delta_g]+fracnu*delta_ur+rho_m_over_rho_r*(fracb*ppw->pv->y[ppw->pv->index_pt_delta_b]+fraccdm*delta_cdm+fracidm_dr*ppw->pv->y[ppw->pv->index_pt_delta_idm_dr]))/(1.+rho_m_over_rho_r);
