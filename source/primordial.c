...
int primordial_omega_gw_at_f(
  ...
  if (ppm->gwb_source_type == PBH_gwb) {
    ...
    class_call(primordial_PBH_omega_gw(ppm,
                                        exp(lnf),
                                        output),
                ppm->error_message,
                ppm->error_message);
...
int primordial_init(
  ...
  if ((ppm->has_gwi == _TRUE_) || (ppm->has_OmGW == _TRUE_)) {
    ...
    class_test(ppm->has_OmGW == _FALSE_,
      ...
      class_call(primordial_PT_gwb_init(ppr,pba,ppt,ppm),
                ppm->error_message,
                ppm->error_message);
    }
  ...
  if ((ppm->has_gwi == _TRUE_) && (ppm->gwb_source_type != analytic_gwb))  {
...
int primordial_free(
  ...
  if (ppm->lnk_size > 0) {
    ...
    if ((ppm->primordial_spec_type == analytic_Pk) || ((ppm->has_gwi == _TRUE_) && (ppm->gwb_source_type == analytic_gwb))) {
      for (index_md = 0; index_md < ppm->md_size; index_md++) {
  ...
  for (index_md = 0; index_md < ppm->md_size; index_md++) {
    ...
    for (index_ic1 = 0; index_ic1 < ppm->ic_size[index_md]; index_ic1++) {
      for (index_ic2 = index_ic1+1; index_ic2 < ppm->ic_size[index_md]; index_ic2++) {
        ...
        /* The GWB inital spectrum is handeled in a different function! */
        if ((ppt->has_gwi == _TRUE_) && ((index_ic1 == ppt->index_ic_gwi)||(index_ic2 == ppt->index_ic_gwi))) {
          ppm->is_non_zero[index_md][index_ic1_ic2] = _FALSE_;
        }
      }
    }
...
int primordial_PBH_gwb_init(
  ...
  int index_f, last_index_back;
  double OmGW, aHtau2;
  double dlnf,lnOmGW_minus,lnOmGW_plus;
  double * pvecback;
  ...
  class_test(ppt->tau_ini_gwb == 0.,
             ppm->error_message,
             "Please use a GWB generation time tau_ini_gwb>0 for PBHs.");
  ...
  class_call(background_at_tau(pba,
              ...
              pba->error_message,
              ppt->error_message);
  aHtau2 = pvecback[pba->index_bg_a]*pvecback[pba->index_bg_a] 
            * pvecback[pba->index_bg_H]*pvecback[pba->index_bg_H]
            * ppt->tau_ini_gwb*ppt->tau_ini_gwb;
...
/**
 * This routine calculates the GWB generated by a primordial phase transition (PT).
 *
 * @param ppr  Input: pointer to precision structure
 * @param pba  Input: pointer to background structure
 * @param ppt  Input: pointer to perturbation structure
...
int primordial_PT_gwb_init(
  ...
  class_test(ppm->has_OmGW == _FALSE_,
             ppm->error_message,
             "You try to calculate the PT_gwb without having a GWB, this should not happen!");

  /** - calculate peak frequency f_co in Hz*/

  f_co = 1.65e-5 * pow(8*_PI_,1./3.) / ppm->RH_star
          * (0.62 * ppm->v_PT / (1.8 - 0.1*ppm->v_PT + ppm->v_PT*ppm->v_PT))
          * (ppm->T_star/1e5) * pow(ppm->g_star/100., 1./6.);
  
  if (ppm->primordial_verbose > 1)
    printf(" -> peak frequency f_co=%g Hz\n",f_co);

  /** - calculate \f$ \Omega_\mathrm{GW} \f$ */
  for (index_f = 0; index_f < ppm->lnf_size; index_f++) {
    f = exp(ppm->lnf[index_f]);

    OmGW = 1.62e-5 / pba->h / pba->h
            * pow(ppm->RH_star / pow(8*_PI_,1./3.), 2)
            * pow(ppm->kappa_phi * ppm->alpha_PT / (1.+ppm->alpha_PT), 2)
            * pow(100. / ppm->g_star, 1./3.)
            * 0.11*ppm->v_PT / (0.42 + ppm->v_PT*ppm->v_PT)
            * 3.8*pow(f/f_co, 2.8) / (1 + 2.8*pow(f/f_co, 3.8));
