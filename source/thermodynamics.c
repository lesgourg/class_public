...
int thermodynamics_recombination(
  ...
  if (pth->recombination==recfast) {
    ...
    class_call(thermodynamics_recombination_with_recfast(ppr,pba,pth,preco,ptw,pvecback),
               pth->error_message,
               pth->error_message);
...
int thermodynamics_recombination_with_recfast(
  ...
  for(i=0; i <Nz; i++) {
    mz_output[i] = -zinitial * (double)(Nz-1-i) / (double)Nz;
  }
  ...
  for (index_interval=0; index_interval<interval_number; index_interval++) {
    ...
    for (i=0; i<ptw->tv->tv_size; i++){
      printf("y[%i] = %e \n",i,ptw->tv->y[i]);
      printf("dy[%i] = %e \n",i,ptw->tv->dy[i]);
      printf("used_in_output[%i] = %s \n",i,(ptw->tv->used_in_output[i]==_TRUE_?"yes":"no"));
    }
...
int thermodynamics_derivs_with_recfast(
  ...
  double z;
  double x,n,n_He,Trad,Tmat,x_H,x_He,dx_H,dx_He,Hz,dHdz,epsilon;
  ...
  */
   ...
   * */
  printf("\n\n");
  printf("Require = %s and %s  ----- ",(ptw->require_H?"H yes":"H no"),(ptw->require_He?"He yes":"He no"));
  if(ptw->require_He){
  ...
  if(ptw->require_H){
    ...
    printf("T_MAT ===  %.10e \n",Tmat);
    printf("Hydrogen ionization first,second : %.10e , %.10e \n",pow((Tmat/1.e4),_b_PPB_),pow((Tmat/1.e4),_d_PPB_));
    ...
    K = preco->CK/Hz;
    printf("Hydrogen ionization Rd,Ru : %.10e , %.10e \n",Rdown,Rup);
    ...
    printf("Hydrogen ionization D : %.10e  , from (%.10e - %.10e)*%.10e + %.10e \n",dy[ptw->tv->index_x_H],
            x*x_H*n*Rdown,Rup*(1.-x_H)*exp(-preco->CL/Tmat), C / (Hz*(1.+z)),-energy_rate*chi_ion_H/n*(1./_L_H_ion_+(1.-C)/_L_H_alpha_)/(_h_P_*_c_*Hz*(1.+z)));
  ...
  if (timeTh < preco->H_frac*timeH) {
    ...
    printf("dT = %.10e (from %.10e , %.10e , %.10e)\n",dy[ptw->tv->index_Tmat],x / (1.+x+preco->fHe) * (Tmat-Trad),2.*Tmat/(1.+z),-2./(3.*_k_B_)*energy_rate*chi_heat/n/(1.+preco->fHe+x)/(Hz*(1.+z)));
...
int thermodynamics_x_analytic(
  ...
  double x_H, x_He, x, rhs,dx_H,dx_He;
  ...
  /** - --> second approximation: first Helium recombination (analytic approximation) */
    ...
    x = 0.5*(sqrt(pow((rhs-1.-preco->fHe),2) + 4.*(1.+2.*preco->fHe)*rhs) - (rhs-1.-preco->fHe));
    dx_H=0.;
    dx_He=0.;
  ...
  /** - --> fifth approximation: Hydrogen recombination starts */
    ...
    x_H = 0.5*(sqrt(pow(rhs,2)+4.*rhs) - rhs);
    dx_H=0.;
