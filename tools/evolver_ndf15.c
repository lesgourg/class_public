...
int generic_evolver(
  ...
  while (done==_FALSE_){
	...
	*/
	...
	for(j=1;j<=neq;j++){
	  ...
	  minnrm = max(minnrm,100*eps*fabs(ynew[j]*invwt[j]));
	}
	...
	for(iter=1;iter<=maxit;iter++){
	  ...
	  newnrm = 0.0;
	  for(j=1;j<=neq;j++){
	    newnrm = max(newnrm,fabs(del[j]*invwt[j]));
	  }
	  for(j=1;j<=neq;j++){
	    difkp1[j] += del[j];
	...
	class_call((*output)(t_vec[next],yinterp+1,ypinterp+1,next,parameters_and_workspace_for_derivs,error_message),
		   error_message,error_message);
      }
      next++;
    }
  ...
  /*Save the last values and return them. Use tnew and ynew since loop may have been broken before update*/
  x_ini = tnew;

  /* added by JL */
  class_call((*derivs)(tnew,ynew+1,f0+1,parameters_and_workspace_for_derivs,error_message),error_message,error_message);
...
/* "interp_from_dif", "eqvec", "adjust_stepsize",                     */
...
int new_linearisation(struct jacobian *jac,double hinvGak,int neq,ErrorMsg error_message){
  ...
  if(jac->use_sparse==1){
    ...
    /* Matrix constructed... */
    if(jac->repeated_pattern<1){
      /*I have a new pattern, so I need to do a full sparse LU-decomposition: */
...
int numjac(
  ...
  */
  ...
  int colmax,group,row,nz,pattern_broken;
  ...
  */
  ...
  if ((jac->use_sparse)&&(jac->repeated_pattern < jac->trust_sparse)){
	...
	if ((i==j)||(fabs(dFdy[i][j])!=0.0)){
	  ...
	  /* Test pattern: */
	  if ((jac->has_pattern)&&(Ai[nz]!=(i-1))) pattern_broken = 1;
	  /* Write row_number: */
	  Ai[nz] = i-1;
	  /* Write value: */
      ...
      if (jac->use_sparse==0) break;
      /*Update Ap and test pattern again. */
      if ((jac->has_pattern)&&(Ap[j]!=nz)) pattern_broken = 1;
      Ap[j]=nz;
    }
    if (jac->use_sparse==1){
      if ((jac->has_pattern)&&(pattern_broken==0)){
	/*I have found the exact same pattern as last time!*/
