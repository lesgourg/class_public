...
int extrapolate_source(
  ...
  for(index_extr=k_size_original;index_extr<k_size_extrapolated;++index_extr){
    switch(extrapolation_method){
      ...
      {
        source_extrapolated[index_extr]=source_max*(log(1.0/h*k_extrapolated[index_extr])/log(1.0/h*k_max));
        break;
      }
      /**
      ...
      {
        //Extrapolation formula taken from HM_Code (and simplified)
...
int extrapolate_k(
  ...
  index_running_k=0;
  running_k = 0.0; /**< Here we make sure that the running_k variable is set,
                        and for any positive k_max_extrapolated will run through the loop*/
