...
//====================================================================================================================
int init_PDE_xarr_HeI(double *xarr, double xmin, double xmax, double xres, double xD, int mess=0)
{
    xD*=xres;
    ...
    int ncore=core_HeI*Doppler_core;
    ...
    //===================================================================================   
    int nup=(int)max(min_np_up, enhance_fac_up*dec_HeI*dec_up);
    int nlow=(int)max(min_np_low, dec_HeI*dec_low);
    ...
    {
        cerr << " init_PDE_xarr_HeI:: grid non-monotonic " << k-1 << " " << xarr[k-1] << endl;
...
// arrangement of grid
//====================================================================================================================
void init_PDE_xarr_cores_HeI(vector<double> &xarr, double xmin, double xmax, vector<double> &resonances)
{
    xarr.resize(100000); // just make the array very large for the moment; unused points will be deleted
    
    if(show_messages_HeI>=1) cout << "\n init_PDE_xarr_cores_HeI :: setting up grid. " << endl;
    
    int mess=(show_messages_HeI>=2 ? 1 : 0);
    int nres=resonances.size();
    int np_2s=npts_2s1s_HeI; 
    double xcrit=0.90;
    ...
    if(nres>1) 
    {
        ...
        // Ly-a
        //=============================================================================
        int nused=init_PDE_xarr_HeI(&xarr[istart], xl, xl+dx[0], resonances[0], xD_f_xarr, mess);
        istart+=nused-1;
        ...
        {
            xl+=dx[i-1];
            nused=init_PDE_xarr_HeI(&xarr[istart], xl, xl+dx[i], resonances[i], xD_f_xarr, mess);
            istart+=nused-1;
        }
    }
    else 
    ...
    { cerr << " init_PDE_xarr_cores_HeI::grid non-monotonic " << k << " " << xarr[k] << endl; exit(0); }
    
    if(show_messages_HeI>=1) cout << " init_PDE_xarr_cores_HeI:: done. " << endl;
